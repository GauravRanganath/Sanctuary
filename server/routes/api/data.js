...
async function moveFile(imageFile, filepath) {
  return imageFile
    .mv(filepath)
    .then((data) => {
      return "uploaded";
...
async function uploadFile(imageFile, filename, filepath) {
  const form = new FormData();
  await form.append("data", fs.createReadStream(filepath));
  await form.append("filename", filename);
  return Axios.post("https://api.estuary.tech/content/add", form, {
    headers: {
...
router.post("/upload", async (req, res, next) => {
  ...
  let collection_names = await Axios.get(
    "https://api.estuary.tech/collections",
    config
  );
  collection_names = collection_names.data.map((value) =>
    Object({ name: value.name, id: value.uuid })
  );
  ...
  all_tags.map(async (specific_tags) => {
    specific_tags.map(async (tag) => {
      tag_id = collection_names.find(
        (col_name_id) => col_name_id.name == tag
      ).id;
      post_body = {
...
router.post("/newRequestData", (req, res) => {
  console.log(req.body);
  RequestData.create(req.body)
    .then((requestData) => res.json({ msg: "RequestData added successfully", _id: requestData._id }))
    .catch((err) =>
      res.status(400).json({ error: "Unable to add RequestData" })
    );
});

router.get("/allRequestData", (req, res) => {
  RequestData.find()
